...
# slackbot url
SLACKBOT_URL = "https://hooks.slack.com/triggers/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"
...
SITES = {"GYR3", "IND9", "LAS1", "LAX9", "MEM1", "MQJ1", "SMF3", "TEB9", "FWA4",
         "GYR2", "IAH3", "ORF2", "RDU2", "RFD2", "RMN3", "SBD1", "SCK4", "SWF2", "VGT2"}
...
def update_goals(sites_dict, driver):
    ...
    for site in sites_dict:
        ...
        rows.append({"site": site, "PID Goal": plan_pid, "PR Goal": plan_pr})

    # Write out to goals.csv
    with open("goals.csv", mode="w", newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["site", "PID Goal", "PR Goal"])
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
        ...
        with open("goals.csv", mode="r") as file:
            ...
            for row in reader:
                ...
                if site in sites_dict:
                    sites_dict[site]["pid_carton"] = int(row["PID Goal"])
                    sites_dict[site]["pr_pallet"] = int(row["PR Goal"])
    ...
    for site in sites_dict:
        ...
        for process_name, process_id in processes.items():
            ...
            try:
                ...
                dedup_df = dedup_df[~dedup_df["Function Name"].isin(["Bin Scout Trans", "Cart Runner Trans", "Tote Stacking", "Transfer In Ambssdor", "Transfer In Training"])]
