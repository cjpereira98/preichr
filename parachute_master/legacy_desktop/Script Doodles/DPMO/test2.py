    ...
    try:
        while True:
            try:
                ...
                logging.error(f"Error processing carton or purchase order details: {e}. Refreshing and trying again.")
            ...
            if 'http://fcresearch-na.aka.amazon.com' in link['href']:
                ...
                if failure_reason:
                    ...
                    if 'ITEMS_NOT_ON_PO' in failure_reason_text:
                        ...
                        try:
                            carton_table = WebDriverWait(driver, 20).until(
                                EC.presence_of_element_located((By.ID, 'table-carton-contents'))
                            )
                            sku_link = carton_table.find_element(By.XPATH, ".//tr/td[2]/a")
                            
                            sku = sku_link.text
                            logging.info(f'SKU: {sku}')
                            
                            # Navigate to the SKU link
                            sku_link.click()

                            # Wait for the purchase order table to be visible
                            purchase_order_table = WebDriverWait(driver, 20).until(
                                EC.presence_of_element_located((By.ID, 'table-purchase-order'))
                            )
                            
                            # Retry logic for stale element reference
                            while True:
                                try:
                                    rows = purchase_order_table.find_elements(By.TAG_NAME, 'tr')
                                    # Filter rows with valid 'data-millis' and find the most recent
                                    rows_with_millis = [row for row in rows if row.get_attribute('data-millis')]
                                    most_recent_order = max(rows_with_millis, key=lambda row: int(row.get_attribute('data-millis')))
                                    logging.info(f"Most recent order: {most_recent_order}")
                                    purchase_order_id = most_recent_order.find_element(By.XPATH, "./td[1]/a").text
                                    logging.info(f'Purchase Order ID: {purchase_order_id}')
                                    break  # Exit the loop if successful
                                except StaleElementReferenceException:
                                    logging.warning("Encountered stale element, retrying...")
                                    continue  # Retry if the element is stale
                        except (NoSuchElementException, TimeoutException) as e:
                            logging.error(f"Error processing carton or purchase order details: {e}")
