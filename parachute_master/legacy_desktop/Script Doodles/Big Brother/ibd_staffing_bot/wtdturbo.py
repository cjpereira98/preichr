...
# sites
SITES = [["GYR3", "IND9", "LAS1", "FTW1"], ["LAX9", "MEM1", "MQJ1", "CLT2"], ["SMF3", "TEB9", "FWA4", "ONT8"],
         ["GYR2", "IAH3", "ORF2", "LGB8"], ["RDU2", "RFD2", "RMN3", "MDW2"], ["SBD1", "SCK4", "SWF2"], ["VGT2","ABE8", "MCC1","AVP1"]]# "ABE8", "AVP1", "FTW1", "CLT2", "ONT8", "LGB8", "MDW2"}
#SITES = [["TEB9"]]
...
def calculate_recommended_staffing(site, sites_dict, driver):
    ...
    date_list = [(start_of_week + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(today.weekday()+1)]
...
def pull_actuals(site, sites_dict, driver):
    ...
    today = datetime.now()
    start_of_last_week = today - timedelta(days=today.weekday() + 1)  # Modified to be this sunday
    start_of_this_week = today #start_of_last_week + timedelta(days=7)
...
def output_to_csv(sites_dict):
    ...
    output_file = "wtd_staffing_results_turbo.csv"
