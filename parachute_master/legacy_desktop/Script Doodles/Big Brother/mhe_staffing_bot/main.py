...
# slackbot url
SLACKBOT_URL = "https://hooks.slack.com/triggers/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"
...
SITES = {"GYR3", "IND9", "LAS1", "LAX9", "MEM1", "MQJ1", "SMF3", "TEB9", "FWA4", "MCC1",
         "GYR2", "IAH3", "ORF2", "RDU2", "RFD2", "RMN3", "SBD1", "SCK4", "SWF2", "VGT2", "CLT2", "FTW1", "LGB8", "ABE8", "MDW2", "AVP1", "ONT8"}

RATE_EXPECTATION = 350
GEN_2_RATE_EXPECTATION = 450
LEGACY_RATE_EXPECTATION = 450
GEN_4_2_RATE_EXPECTATION = 400
            ...
            for request in driver.requests:
                if request.response and plan_url in request.url:
                    try:
                        response_json = request.response.body.decode('utf-8')
                        plan_data = json.loads(response_json)
                        #plan_to_jobs = plan_data[0]['data'][5]['subfunctions'][1].get("plan")
                        plan_pid = plan_data[0]['data'][1]['subfunctions'][7].get("plan")
                        plan_presort = plan_data[0]['data'][4]['subfunctions'][0].get("plan")
                        plan_online_cartons = plan_pid + plan_presort
                        break
                    except Exception:
                        continue
            if plan_online_cartons > 0:
                break

        rows.append({"site": site, "Online Cartons Goal": plan_online_cartons})

    # Write out to goals.csv
    with open("goals.csv", mode="w", newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["site", "Online Cartons Goal"])
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
        ...
        with open("goals.csv", mode="r") as file:
            ...
            for row in reader:
                ...
                if site in sites_dict:
                    sites_dict[site]["online_cartons"] = int(float(row["Online Cartons Goal"]))
    except FileNotFoundError:
        print("goals.csv not found. Using placeholder goals instead.")
        for site in sites_dict:
            sites_dict[site]["online_cartons"] = 50000
    return sites_dict
...
def calculate_recommended_staffing(sites_dict):
    ...
    for site in sites_dict:
        # Lookup from matrix
        key = (site)
        if site in {"ABE8", "LGB8", "FTW1", "CLT2", "MDW2"}:
            rate = GEN_2_RATE_EXPECTATION
        elif site in {"AVP1","ONT8"}:
            rate = LEGACY_RATE_EXPECTATION
        elif site in {"GYR3", "IND9", "LAS1", "LAX9", "MEM1", "MQJ1", "SMF3", "TEB9"}:
            rate = GEN_4_2_RATE_EXPECTATION
        else:
            rate = RATE_EXPECTATION
        sites_dict[site]["recommended_staffing"] = int(sites_dict[site]["online_cartons"]/10/rate)  # Default to 0 if missing
    ...
    for site in sites_dict:
        ...
        for process_name, process_id in processes.items():
            ...
            try:
                ...
                uplift_funcs = ["MHE Support", "Bin Scout Trans"]
                uplift_df = dedup_df[dedup_df["Function Name"].isin(uplift_funcs)]
                ...
                if "Paid Hours-Total(function,employee)" in uplift_df.columns:
                    hours = uplift_df["Paid Hours-Total(function,employee)"].sum()
                    total_hours += hours
                else:
...
def format_slack_message(sites_dict):
    ...
    """
    header = f"{'Site':<6} {'BU HC':>8} {'Actual HC':>10} {'HC Δ':>8}"
