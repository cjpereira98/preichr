...
#SITES = {"TEB9"}
#SITES = [["ONT8"],["FWA4"],["MQJ1"],["CLT2"], ["ORF2"],["ONT8"]]
        ...
        for process_name, process_id in processes.items():
            ...
            for attempt in range(10):
                url = (
                    ...
                    f"&spanType=Month&startDateMonth=2025/{month}/01&maxIntradayDays=1&startHourIntraday=0"
                    f"&startMinuteIntraday=0&endHourIntraday=0"
                    f"&endMinuteIntraday=0"
                )

                # Take a snapshot of existing matching files before requesting a new one
                download_dir = os.getcwd()
                prefix = f"functionRollupReport-{site}"
                pattern = os.path.join(download_dir, f"{prefix}*.csv")
                preexisting = set(glob.glob(pattern))

                driver.execute_script(f"window.open('{url}', '_blank');")

                # Wait for a *new* matching file to appear
                filename = None
                filepath = None
                if test_mode:
                    print(f"[INFO] Attempt {attempt + 1}: Waiting for file matching {prefix}*.csv from {url}")

                for _ in range(60):  # up to 60 seconds
                    current = set(glob.glob(pattern))
                    new_files = list(current - preexisting)
                    if new_files:
                        # If multiple land, grab the newest by mtime
                ...
                if not filepath or not os.path.exists(filepath):
                    print(f"[WARN] Attempt {attempt + 1}: No new file matching {prefix}*.csv found")
                    continue
