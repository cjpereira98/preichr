...
class RCSortHours(Metric):
    ...
    def fetch_and_sum_hours(self, start: datetime, end: datetime):
        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[3] == 'RC Sort - Total':  # Column D = 'Prep - Total'
                    try:
                        total += float(row[8])  # Column I contains the hours
                    except ValueError:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = rcsh.get(specificity=specificity, start=start, end=end)
