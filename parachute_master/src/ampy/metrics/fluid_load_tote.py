...
class FluidLoadTote(Metric):
    ...
    def fetch_and_sum_data(self, start: datetime, end: datetime):
        ...
        # Generate URL
        url = f"https://fclm-portal.amazon.com/reports/unitsRollup?reportFormat=CSV&warehouseId=SWF2&jobAction=FluidLoadTote&startDate={start_date}&startHour={start_hour}&startMinute=0&endDate={end_date}&endHour={end_hour}&endMinute=0"
        ...
        download_filename = f"unitsRollup-SWF2-FluidLoadTote-{start_str_utc}-{end_str_utc}.csv"
        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[6] == 'Tote':  # Checking if the value in column G is 'Tote'
                    try:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = fluid_load_tote.get(specificity=specificity, start=start, end=end)
