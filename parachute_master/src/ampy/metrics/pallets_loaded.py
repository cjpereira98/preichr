...
class PalletsLoaded(Metric):
    ...
    def fetch_and_sum_data(self, start: datetime, end: datetime):
        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[6] == 'Pallet':  # Checking if the value in column G is 'Case'
                    try:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = pl.get(specificity=specificity, start=start, end=end)
