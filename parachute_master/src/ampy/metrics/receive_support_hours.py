...
class ReceiveSupportHours(Metric):
    ...
    def fetch_and_sum_hours(self, start: datetime, end: datetime):
        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[3] == 'Receive Support':  # Column D = 'Receive Support'
                    try:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = rs_hours.get(specificity=specificity, start=start, end=end)
