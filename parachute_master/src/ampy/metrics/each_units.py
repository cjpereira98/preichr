        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[3] == 'Each Receive - Total':  # Column D = 'Each Receive - Total'
                    try:
                        total_hours += float(row[7])  # Column H contains the units
                    except ValueError:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = each_units.get(specificity=specificity, start=start, end=end)
