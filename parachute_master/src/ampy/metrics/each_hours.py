...
class EachHours(Metric):
    ...
    def fetch_and_sum_hours(self, start: datetime, end: datetime):
        ...
        #start_localized = start.astimezone(pytz.timezone('America/New_York'))
        start_str_utc = start.astimezone(pytz.UTC).strftime('%Y%m%d%H%M%S')
        #end_localized = end.astimezone(pytz.timezone('America/New_York'))
        ...
        with open(download_path, 'r') as csvfile:
            ...
            for row in reader:
                if len(row) > 8 and row[3] == 'Each Receive - Total':  # Column D = 'Each Receive - Total'
                    try:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = each_hours.get(specificity=specificity, start=start, end=end)
