...
from metrics.ib_cplh import IBCPLH
from metrics.receive_cplh import ReceiveCPLH
from metrics.receive_support_cplh import ReceiveSupportCPLH
from metrics.decant_uph import DecantUPH
from metrics.manual_edit_hours import ManualEditHours
from metrics.hostler_moves_per_hour import HostlerMovesPerHour
...
metric_names = [
    ...
    'ReceiveCPLH', 'ReceiveSupportCPLH', 'DecantUPH', 'ManualEditHours', 
    'HostlerMovesPerHour'
]
...
def populate_metrics(ws, driver):
    ...
    # Precompute PIDCartons and PRCartons values for each shift
    pid_values = {}
    pr_values = {}
    for shift in shifts:
        pid_values[shift] = PIDCartons(driver).get(shift)
        pr_values[shift] = PRCartons(driver).get(shift)
    
    # Dynamically create instances of each metric class
    metric_instances = {}
    for metric_name in metric_names:
        metric_class = globals().get(metric_name)
        if metric_class:
            metric_instances[metric_name] = metric_class(driver)
    ...
    for col_letter, metric_name in zip(metric_columns, metric_names):
        ...
        if metric_instance:
            for shift, row in shifts.items():
                ...
                if metric_name == 'CombinedCartons':
                    # Pass precomputed values for CombinedCartons
                    ws[cell] = metric_instance.get(shift, pid_value=pid_values[shift], pr_value=pr_values[shift])
                else:
