...
def generate_ti_report():
    # File paths
    csv_file_path = os.path.join(os.path.dirname(__file__), 'AFT Transshipment Hub.csv')
    ...
    <body>
        <h2>TI Report</h2>
        <h3>Total Freight</h3>
        {create_table(all_freight)}
        <br>
        <h3>Floor Loaded Freight</h3>
        {create_table(floor_loaded)}
        <br>
        <h3>Palletized Freight</h3>
        {create_table(palletized)}
    </body>
    </html>
    """
    ...
    outlook.create_html_email(
        ...
        subject='TI Report ' + datetime.now().strftime('%Y-%m-%d'),
