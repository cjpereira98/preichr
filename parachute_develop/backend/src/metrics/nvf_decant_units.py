...
class NVFDecantUnits(Metric):
    ...
    def aggregate_shift(self, shift: str):
        ...
        for day, start_hour, end_hour in shift_times[shift]:
            start_dt = datetime.combine(day, datetime.min.time()) + timedelta(hours=start_hour)
    ...
    def fetch_and_sum_data(self, start: datetime, end: datetime):
        ...
        url = f"https://fclm-portal.amazon.com/reports/functionRollup?reportFormat=CSV&warehouseId=SWF2&processId=1003033&maxIntradayDays=1&spanType=Intraday&startDateIntraday={start_date_str}&startHourIntraday={start_hour}&startMinuteIntraday={start_minute}&endDateIntraday={end_date_str}&endHourIntraday={end_hour}&endMinuteIntraday={end_minute}"

        # Execute JavaScript to download the file directly
        self.driver.execute_script(f"window.open('{url}', '_blank');")
        #print("I made it!")

        modified_start = start + timedelta(hours=4)
        modified_end = end + timedelta(hours=4)

        file_name_start = modified_start.strftime("functionRollupReport-SWF2-Receive-Support-Intraday-%Y%m%d%H%M00-")
            ...
            for i, row in enumerate(reader):
                if i >= 2 and len(row) > 16:  # Ensure the row has enough columns to avoid out-of-range errors
                    try:
                        ...
                        if row[1].strip() == 'Decant Non-TI' and row[14].strip() == 'EACH' and row[15].strip() == 'Total':
                            total_value += float(row[16])  # Add the value in column Q
                    except ValueError:
...
def main():
    ...
    for specificity in specificities:
        ...
        result = units.get(specificity=specificity, start=start, end=end)
