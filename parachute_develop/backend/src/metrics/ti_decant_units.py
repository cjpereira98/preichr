...
class TIDecantUnits(Metric):
    ...
    def fetch_and_sum_data(self, start: datetime, end: datetime):
        ...
        file_name_start = modified_start.strftime("functionRollupReport-SWF2-Transfer In Dock-Intraday-%Y%m%d%H%M00-")
            ...
            for i, row in enumerate(reader):
                if i >= 2 and len(row) > 16:  # Ensure the row has enough columns to avoid out-of-range errors
                    try:
                        ...
                        if row[1].strip() == 'Decant' and row[14].strip() == 'EACH' and row[15].strip() == 'Total':
